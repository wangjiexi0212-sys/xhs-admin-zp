        ...
        <a-row :gutter="16">
          ...
          <a-col :span="24">
            <a-form-item label="模板标题" name="title">
              <a-input v-model:value="form.title" placeholder="请输入模板标题" :maxlength="100" />
            </a-form-item>
          </a-col>
          <a-col :span="24">
...
const columns = [
  ...
  { title: '模板标题', dataIndex: 'title', width: 200 },
...
async function fetchList() {
  ...
  try {
    ...
    const res = await getContentTemplateList(params)
    list.value = res.list ?? []
...
const rules = {
  ...
  title: [{ required: true, message: '请输入模板标题', whitespace: true }],
  content: [{ required: true, message: '请输入模板内容', whitespace: true }],
}
...
async function onSubmit() {
  ...
  try {
    ...
    if (editingId.value) {
      await updateContentTemplate(editingId.value, payload)
